...
INT_PTR OptionsPage::s_DlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
	OptionsPage* pPage;
    /* TODO(jec)
	switch (uMsg) {
...
BOOL OptionsPage_Instrument::OnNotify(HWND hPage, DWORD ctrlId, const LPNMHDR pNmHdr)
{
    /* TODO(jec)
...
BOOL OptionsPage_Joystick::OnInitDialog(HWND hPage, WPARAM wParam, LPARAM lParam)
{
	...
	DWORD ndev;
    /* TODO(jec):  Also add joystick enumeration capability to I_Input
...
BOOL OptionsPage_CelSphere::OnNotify(HWND hPage, DWORD ctrlId, const LPNMHDR pNmHdr)
{
    /* TODO(jec)
	if (pNmHdr->code == UDN_DELTAPOS) {
		NMUPDOWN* nmud = (NMUPDOWN*)pNmHdr;
...
void OptionsPage_Planetarium::RescanMarkerList(HWND hPage)
{
    ...
    */
	...
	bool found = g_psys->FindFirst("mkr", dir_it, fname);
	if (found) {
		int n = 0;
		do {
            /* TODO(jec)
            ...
            */
			n++;
		} while (g_psys->FindNext("mkr", dir_it, fname));
	}
}
...
void OptionsPage_Labels::UpdateFeatureList(HWND hPage)
{
    ...
    */
	...
	if (planet->LabelFormat() < 2) {
		...
		if (!nlist) return;
        std::filesystem::directory_iterator dir_it;
		bool found = planet->FindFirst("mkr", dir_it, cpath, fname);
		if (found) {
			n = 0;
			do {
                /* TODO(jec)
				SendDlgItemMessage(hPage, IDC_OPT_MKR_FEATURELIST, LB_ADDSTRING, 0, (LPARAM)trim_string(fname));
				if (n < nlist && list[n].active)
					SendDlgItemMessage(hPage, IDC_OPT_MKR_FEATURELIST, LB_SETSEL, TRUE, n);
                */
				n++;
			} while (planet->FindNext("mkr", dir_it, fname));
		}
	}
	else {
