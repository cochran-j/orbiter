...
bool XRSoundEngine::InitializeIrrKlangEngine()
{
    ...
    if (!XRSoundEngine::IsKlangEngineInitialized())
    {
        ...
        char logMsg[256];   // can't use string easily here b/c Orbiter's oapiWriteLog takes a char * instead of const char * for some bizarre reason.
        if (s_pKlangEngine)
            std::snprintf(logMsg, 256, "%s initialized using sound driver %s; irrKlang version = %s.  XRSound UpdateInterval = %.03lf (%.1lf updates per second)", 
            ...
            std::snprintf(logMsg, 256, "%s ERROR: could not initialize default sound device.", GetVersionStr());
...
{
    char logMsg[256];   // can't use string easily here b/c Orbiter's oapiWriteLog takes a char * instead of const char * for some bizarre reason.
    std::snprintf(logMsg, 256, "%s terminating.", GetVersionStr());
...
bool XRSoundEngine::LoadWav(const int soundID, const std::string& pSoundFilename, const XRSound::PlaybackType playbackType)
{
    if (!IsKlangEngineInitialized())
        return false;
    ...
    if (pContext)
    {
        ...
        // NOTE(jec):  Switching this comparison from case insensitive to
        // case sensitive.  This accomodates non-win32 filesystems.  On win32
        // filesystems, it may cause extra unnecessary loads of the same file
        // to occur if the caller to this function is not careful with cases.
        if (pContext->csSoundFilename == pSoundFilename)
    ...
    if (!ConfigFileParser::IsFileReadable(pSoundFilename.c_str()))
    {
...
void XRSoundEngine::WriteLog(const char *pMsg)
{
    ...
    std::ostringstream msg_os {};
    msg_os << "[" << GetLogID() << "]["
           << std::setprecision(3) << std::setfill('0')
           << XRSoundDLL::GetAbsoluteSimTime()
           << " " << pMsg;
    std::string msg = msg_os.str();
...
const char *XRSoundEngine::GetVersionStr()
{
    if (s_csVersion.empty()) {
        ...
        os << "XRSound "
           << std::setprecision(2) << XRSOUND_ENGINE_VERSION
           << " " << XRSOUND_BETA_STR
           << "[" << ARCH_TYPE
           << " " << BUILD_TYPE
           << "], Build Date : " << __DATE__;
...
bool XRSoundEngine::PauseOrReumseMusic(const bool bPause)
{
    ...
    if (s_pMusicFolderWavContext && !s_pMusicFolderWavContext->bPaused)  
    {
        ...
        if (pISound)
        {
            ...
            if (bPause)
            {
                msg = "Pausing global music slot ";
                msg += s_pMusicFolderWavContext->ToStr();

                pISound->setIsPaused(true);
            }
            else
            {
                msg = "Unpausing global music slot ";
                msg += s_pMusicFolderWavContext->ToStr();

                pISound->setIsPaused(false);
            }
            XRSoundDLL::WriteLog(msg.c_str());
        }
    }
