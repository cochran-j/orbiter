...
public:
    ...
    // Note: if a string filename / file path is empty, it means that default sound is disabled.
    // Therefore, we do no initialize these string values to a default value in the constructor.
    ...
    std::vector<std::string> SupportedSoundFileTypesAsVector() const
    {
        ...
        std::string::size_type tokPos = 0;
        tokPos= SupportedSoundFileTypes.find(pTokenStr, strPos);
        while (tokPos != std::string::npos)
        {
            vec.push_back(SupportedSoundFileTypes.substr(strPos, tokPos));
            strPos = tokPos + 1;
            tokPos = SupportedSoundFileTypes.find(pTokenStr, strPos);
        }
        return vec;
    ...
    std::vector<int> CustomEnginesThrusterIndexesAsVector() const
    {
        ...
        tokPos = CustomEnginesThrusterIndexes.find(pTokenStr, strPos);
        while (tokPos != std::string::npos)
        {
            vec.push_back(std::stoi(CustomEnginesThrusterIndexes.substr(strPos, tokPos)));  // will be 0 if invalid integer, but that's good enough
                                                                                            strPos = tokPos + 1;
            tokPos = CustomEnginesThrusterIndexes.find(pTokenStr, strPos);
        }
        return vec;
